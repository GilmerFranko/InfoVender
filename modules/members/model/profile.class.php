...
class Profile extends Model
{
    ...
    function getMemberProfile($user = '')
    {
        ...
        $query = $this->db->query('SELECT m.`member_id`, m.`name`, m.`group_id`, m.`email`, m.`num_phone`, `birthday`, m.`banned`, m.`last_activity`, m.`last_login`, m.`pp_full_name`, m.`pp_main_photo`, m.`pp_thumb_photo`, m.`pp_photo_type`, m.`pp_setting_preferences`, m.`pp_gender`, m.`pp_joined`, m.`pp_expiration`, r.`g_title`, r.`g_colour` FROM `members` AS m LEFT JOIN `members_groups` AS r ON r.`g_id` = m.`group_id` WHERE ' . $where . ' LIMIT 1');
