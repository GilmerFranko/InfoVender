...
class Transactions extends Session
{
	...
	public function getAllTransactions($perPage = 10)
	{
		...
		if ($queryTransactions && $queryTransactions->num_rows > 0)
		{
			...
			{
				$row['name'] = loadClass('members/member')->getName($row['member_id']);
				$transactions[] = $row;
			}
		}
	 ...
	 */
	public function getTransactions($member_id = null, $page = 1, $perPage = 10)
	{
		if ($member_id === null)
		{
			$member_id = $this->memberData['member_id'];
	 ...
	 * @return array|null Arreglo asociativo con la transacción, o null si no se encuentra
	 */
	public function getTransactionById($transaction_id)
	{
		$query = $this->db->query("SELECT * FROM `members_transactions` WHERE `transaction_id` = '{$transaction_id}' LIMIT 1");

		return $query && $query->num_rows > 0 ? $query->fetch_assoc() : false;
	}
